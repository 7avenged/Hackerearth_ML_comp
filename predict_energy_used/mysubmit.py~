...
X = preprocessing.StandardScaler().fit(X1)
X.transform(X1)   
#X = np.column_stack( X1 )
X = np.column_stack( X )
#Xl = np.column_stack( X2 )
#Xl = preprocessing.scale(X2)
Xl = preprocessing.StandardScaler().fit(X2)
Xl.transform(X2)   
...
d= d.T
d1=d1.T
d = pd.Series(d).round(0)
d1= pd.Series(d1).round(0)
df3 = pd.DataFrame({"Observation" : df2.Observation, "Energy" :d})
...
print(params)
print(accuracy_score(y3, d1, normalize=True) )
#print("xg:")
